...
window.Tutor = (function () {
  ...
  function getLesson(topic) {
    return (window.Lessons && window.Lessons[topic._id]) || {};
  }

  /* Quick chips shown above the chat */
  function suggestions(topic) {
    const base = ["Explain this in simpler words", "Give me another example", "Why is this formula used?", "Explain like I'm a beginner", "Give me a harder question"];
    const ls = getLesson(topic);
    if (ls.formulas && ls.formulas.length) base.push("What does " + ls.formulas[0].name + " mean?");
    if (ls.keyTerms && ls.keyTerms.length) base.push("What is " + ls.keyTerms[0].term + "?");
    if (ls.workedExamples && ls.workedExamples.length) base.push("Walk me through an example");
    if (ls.questions && ls.questions.length) base.push("Quiz me");
    return base.slice(0, 6);
  }

  /* Main answer engine */
  function ask(topic, question) {
    const ls = getLesson(topic);
    ...
    if (/(another example|more example|different example|one more example|show me an example)/.test(q)) {
      ...
      if (ls.applications && ls.applications.length) return "**Real-world example:**\n\n" + ls.applications[0];
    ...
    if (/(quiz|test me|question me|practice)/.test(q)) {
      ...
      const qq = pick(bank);
      let s = "**Quick question** (" + (Quiz.TYPE_LABELS[qq.type] || "question") + "):\n\n" + qq.q;
      if (qq.type === "mcq") s += "\n\n" + qq.options.map((o, i) => String.fromCharCode(65 + i) + ". " + o).join("\n");
      return s;
    }
    ...
    if (/(formula|calculate|equation|solve)/.test(q)) {
      if (ls.formulas && ls.formulas.length) {
